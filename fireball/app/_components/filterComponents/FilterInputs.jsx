...
export default function FilterInputs(props) {
  ...
  function filterDataSet(nameInput, recInput) {
    const filteredData = [...meteoriteData]
      .filter((meteorObj) => {
        return meteorObj.name.toLowerCase().includes(nameInput.toLowerCase());
      })
      .filter((meteorObj) => {
        return meteorObj.recclass
          .toLowerCase()
          .includes(recInput.toLowerCase());
      });
    ...
    <>
      ...
      <div className="InputSearches">
        <div className="flex flex-col w-40">
          <input
            className="border border-borderColor p-1"
            type="text"
          ...
          />
          <input
            className="border border-borderColor p-1 my-1"
            type="text"

...
export default function Home() {
  ...
  function filterMass(meteoriteData) {
    return (
      meteoriteData.mass >= massSliderV1 && meteoriteData.mass <= massSliderV2
    );
  }
  ...
  function filterName(meteoriteData) {
    return nameSearch.toLowerCase() === ""
      ? meteoriteData
      : meteoriteData.name.toLowerCase().includes(nameSearch.toLowerCase());
  }
  // filter Rec Class with text input
  function filterRecClass(meteoriteData) {
    return recSearch.toLowerCase() === ""
      ? meteoriteData
  ...
  return (
    <>
      <Header />
      <main className="flex-1 lg: lg:grid lg:grid-cols-10 flex flex-col pt-12 px-12 gap-4 min-h-full">
